<?php
	...
	class WiziappPluginModuleAndroid
	{
		function init()
		{
			...
			/** @since 4.2.1 */
			wiziapp_plugin_hook()->hookInstall(array(&$this, 'reset_apk_build_status'));
		}
		...
		function build()
		{
			...
			$response = wp_remote_post($wiziapp_plugin_config['build_host'].'/buildApk', array(
				'body' => array(
					...
					'homescreen' => ( true === wiziapp_plugin_settings()->getAndroidHomescreen() ) ? 'true' : 'false'
				), 'timeout' => 90
			));
			if (is_wp_error($response))
			{
 ...
 */
		...
		/**
		* Restores the ability to upload non-image files in WordPress 4.7.1.
		*
		...
		*/
		...
		/**
		* Fix: APK build status
		*
		* @author ido
		* @since 4.2.1
		*/
		function reset_apk_build_status() {
			if ( ! wiziapp_plugin_settings()->getAndroidDownload() && ! wiziapp_plugin_settings()->getAndroidNeedBuild() && wiziapp_plugin_settings()->getAndroidBuildToken() ) {
				wiziapp_plugin_settings()->setAndroidNeedBuild( true );
			}
		}
	}
