...
<?php
		...
		function admin_enqueue_scripts($page)
		{
			...
			wp_enqueue_media();
			add_thickbox();
			wp_register_style('wiziapp-plugin-admin', wiziapp_plugin_hook()->plugins_url('/styles/admin.css'), array(), $this->plugin_get_version());
			wp_register_script('wiziapp-plugin-admin', wiziapp_plugin_hook()->plugins_url('/scripts/admin.js'), array('wp-plupload'), $this->plugin_get_version());
		...
		public function plugin_get_version() {
			$plugin_data = get_plugin_data( dirname( dirname (__FILE__ ) ) . '/wiziapp.php' );
		...
		function update_settings()
		{
			...
			switch ($_POST['name'])
			{
				...
				case 'android_active':
					...
					break;
				case 'android_homescreen':
					wiziapp_plugin_settings()->setAndroidHomescreen($_POST['value'] === 'true');
					break;
				case 'android_theme':
				...
				case 'android_app':
					...
					$apk_url = add_query_arg( 'SiteUrl', trailingslashit(get_bloginfo('wpurl')), $wiziapp_plugin_config['build_host'] . '/getapk' );

                    if ( '' === wiziapp_plugin_settings()->getAndroidName() || false === wiziapp_plugin_settings()->getAndroidIcon() || false === wiziapp_plugin_settings()->getAndroidSplash() ) {
						$value = 'need-values';
					} elseif ( wiziapp_plugin_settings()->getAndroidNeedBuild() ) {
						$value = 'need-build';
					} elseif ( wiziapp_plugin_settings()->getAndroidBuildToken() ) {
						$value = 'building';
						$headers = wp_get_http_headers( $apk_url );
						if ( $headers && false === strpos( $headers['content-type'], 'application/json' ) ) {
					...
					wiziapp_plugin_hook()->json_output( array( 'value' => $value, 'extra' => $apk_url ) );
					break;
				//Ios
				case 'ios_active':
...
<div class="wiziapp-plugin-admin-container" data-wiziapp-plugin-admin-url="<?php echo esc_attr($admin_base); ?>" data-wiziapp-plugin-analytics-url="<?php echo esc_attr($wiziapp_plugin_config['build_host'].'/analytics?url='.urlencode(trailingslashit(get_bloginfo('wpurl'))).'&page='); ?>">
	<div class="wiziapp-plugin-admin-header">
		...
		<div class="wiziapp-plugin-admin-header-tab">
			<a href="http://wiziapp.com/#contactUs" target="_blank"><?php _e('Support', 'wiziapp-plugin'); ?></a>
		</div>
	</div>
...
<?php
	...
	function _renderTabSettings() {
		...
		$this->_renderOptionsBox( array(
			...
			'items' => array(
				array(
					...
					'description' => __( 'Activate the mobile theme, please note that we sometimes display ads on the free version. Ads can be eliminated by purchasing the Ads module, or the Android App license', 'wiziapp-plugin' ),
				...
				array(
					'id' => 'android_active',
					'type' => 'switch',
					'label' => __( 'Activate Wiziapp powered Themes - Android Native App', 'wiziapp-plugin' ),
		...
		<div id="wiziapp-plugin-admin-settings-box-monetization-body-buy" class="wiziapp-plugin-admin-settings-box">
			...
			<div class="wiziapp-plugin-admin-settings-box-body">
				<div class="wiziapp-plugin-admin-state-buy-billing-description">
					<?php _e('Only for users that choose to activate Wiziapp free mobile themes - We sometimes display ads to help pay the bills. This keeps free features free! To eliminate ads entirely / Take over the Ad Space, you can purchase the monetization module.', 'wiziapp-plugin') ?>
				</div>
		...
		<div class="wiziapp-plugin-admin-state-buy">
			<div class="wiziapp-plugin-admin-state-buy-description">
				<ul class="wiziapp-plugin-admin-features">
					...
					<li class="wiziapp-plugin-admin-state-buy-feature-themes"><?php _e('Keep your own responsive theme.', 'wiziapp-plugin') ?></li>
					<li class="wiziapp-plugin-admin-state-buy-feature-noad"><?php _e('Display your Ads or keep the App free from Ads (Ad Space license included)', 'wiziapp-plugin') ?></li>
                    <li class="wiziapp-plugin-admin-state-buy-feature-create"><?php _e('Just $6.60/month ($79/year Billed annually)', 'wiziapp-plugin') ?></li>
				</ul>
			</div>
			<div class="wiziapp-plugin-admin-state-buy-billing">

                <div class="wiziapp-plugin-admin-settings-container">
                    <div class="wiziapp-plugin-admin-state-buy-billing-simulate wiziapp-plugin-admin-settings-button wiziapp-plugin-admin-state-buy-billing-simulate-device">
                        <a href="http://blog.wiziapp.com/blog/guides-tutorials/wiziapp-android-apps-showcase/" target="_blank"><?php _e('Showcase', 'wiziapp-plugin') ?><span></span></a>
                    </div>
                </div>

				<div class="wiziapp-plugin-admin-settings-container" style="display: flex; flex-direction: column; align-items: center; float: none; width: auto;">
					<div class="wiziapp-plugin-admin-state-buy-billing-price">
						<span class="wiziapp-plugin-admin-state-buy-billing-price-amount"></span><span class="wiziapp-plugin-admin-state-buy-billing-price-duration"><?php _e('/Year', 'wiziapp-plugin') ?></span>
		...
		*/
		...
		$this->_renderOptionsBox( array(
			...
			'items' => array(
				...
				array(
					'id' => 'android_homescreen',
					'type' => 'switch',
					'label' => __( 'Display icon on device home screen', 'wiziapp-plugin' ),
					'description' => '',
					'value' => wiziapp_plugin_settings()->getAndroidHomescreen() 
				),
				array(
					'id' => 'android_name',
					'type' => 'text',
				...
				array(
					...
					'description_link' => 'http://blog.wiziapp.com/blog/guides-tutorials/creating-google-cloud-messaging-for-android/',
				...
				array(
					...
					'description_link_text' => __( 'See How', 'wiziapp-plugin' ),
					'description_link' => 'http://blog.wiziapp.com/blog/guides-tutorials/publishing-your-android-app/',
			...
			<div class="wiziapp-plugin-admin-state-buy-billing">
				...
				<div class="wiziapp-plugin-admin-settings-container">
					<div class="wiziapp-plugin-admin-state-buy-billing-simulate wiziapp-plugin-admin-settings-button">
						<a href="<?php echo esc_attr($admin_base.'customize.php?wiziapp_plugin=customize&theme='.urlencode($webapp_theme).'&return='.urlencode($admin_base.'admin.php?page=wiziapp-plugin-settings')); ?>" title="<?php _e('Web Simulator', 'wiziapp-plugin') ?>"><?php _e('Simulate the App - Web Simulator', 'wiziapp-plugin') ?><span></span></a>
					</div>
					<div class="wiziapp-plugin-admin-state-buy-billing-simulate wiziapp-plugin-admin-settings-button wiziapp-plugin-admin-state-buy-billing-simulate-device">
						<a href="https://itunes.apple.com/us/app/wiziapp/id1217974809?mt=8" target="_blank" title="<?php _e('Native IOS App', 'wiziapp-plugin') ?>"><?php _e('Simulate the App - iOS Device', 'wiziapp-plugin') ?><span></span></a>
					</div>
				</div>
		...
		<div class="wiziapp-plugin-admin-state-buy">
			<div class="wiziapp-plugin-admin-state-buy-description">
				<ul class="wiziapp-plugin-admin-features">
					<div calss="header" ><?php _e('Get all in One Pack and Save $99/Year!') ?> </div>
		...
		*/
		...
		?>
		<div id="wiziapp-plugin-admin-rate-box" style="display: none;">
			...
			<div class="wiziapp-plugin-ajax-loader-container">
				<div class="wiziapp-plugin-admin-billing-type-details">
					...
					<div class="wiziapp-plugin-admin-billing-type-details-terms">
						<input type="checkbox" /> <span class="wiziapp-plugin-admin-billing-type-details-label"><?php _e('I accept WiziApp ', 'wiziapp-plugin') ?><a href="http://wiziapp.com/TermsAndConditions.html" target="_blank"><?php _e('Terms &amp; Conditions', 'wiziapp-plugin') ?></a></span>
					</div>
				</div>
